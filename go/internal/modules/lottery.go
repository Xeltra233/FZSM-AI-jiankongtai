...
func RunLottery(cfg *config.Config, st *storage.Storage, c *client.Client, values map[string]any) map[string]any {
	...
	if st != nil {
		...
		prefSafe := st.GetStateMap("vip.room_pref")
		outPref := map[string]any{}
		for k, v := range prefSafe {
			if k == "password" {
				continue
			}
			outPref[k] = v
		}
		pwd := strings.TrimSpace(fmt.Sprint(prefSafe["password"]))
		outPref["has_password"] = pwd != "" && pwd != "<nil>" && pwd != "null"
		analysis["vip_room_pref"] = outPref
	...
	if autoVip {
		...
		if !asBool(vipState["can_enter"], false) && !allowSpectator {
			...
			joinPwd := strings.TrimSpace(fmt.Sprint(firstNonNil(selected["_password"], selected["password"], "")))
			if joinPwd == "" && st != nil {
				pref := st.GetStateMap("vip.room_pref")
				if strings.TrimSpace(fmt.Sprint(firstNonNil(pref["room_id"], pref["id"], ""))) == strings.TrimSpace(fmt.Sprint(rid)) {
					joinPwd = strings.TrimSpace(fmt.Sprint(pref["password"]))
				}
			}
			if (asBool(selected["has_password"], false) || asBool(selected["_needs_password"], false)) && joinPwd == "" {
				actions = append(actions, map[string]any{
					"status": "skip", "action": "vip_join", "reason": "vip_password_required",
					"detail": "密码房需要先在面板输入房间密码", "room_id": rid,
					"edge": vipBetEdge, "selected": selected, "context": vipCtx,
				})
...
func pickVipRoom(st *storage.Storage, state, ctx map[string]any, balance float64) map[string]any {
	...
	if prefID != "" && prefID != "<nil>" && prefID != "null" && prefID != "0" {
		...
		for _, it := range rooms {
			...
			needPwd := asBool(m["has_password"], false)
			prefPwd := strings.TrimSpace(fmt.Sprint(pref["password"]))
			if needPwd && prefPwd == "" {
				out := map[string]any{}
				for k, v := range m {
					out[k] = v
			...
			out["_strategy"] = "manual_pref"
			if needPwd {
				out["_password"] = prefPwd
				out["_has_password"] = true
				out["has_password"] = true
			}
			return out
