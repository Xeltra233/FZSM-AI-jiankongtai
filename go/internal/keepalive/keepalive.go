...
func Run(cfg *config.Config, st *storage.Storage, c *client.Client, force bool, cycle int) map[string]any {
        ...
        ok := asBool(stocks["ok"]) && (asBool(lottery["ok"]) || asBool(lottery["skipped"]))
        msg := "cookie 保活正常"
        if !ok {
                msg = "cookie 保活失败，需要手动续登"
        }
        state := map[string]any{
                "enabled": true, "ok": ok, "ts": now(), "cycle": cycle, "last_cycle": cycle,
                ...
                "manual_reauth_hint": nil, "prev_ok": prev["ok"], "impl": "go",
        }
        if !ok {
                state["manual_reauth_hint"] = "python -m src.auth_helper login 或 import-json"
