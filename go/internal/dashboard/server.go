...
func (s *Server) fundsBreakdown(account map[string]any) map[string]any {
        ...
        for _, p0 := range positions {
                ...
                if mv == 0 && shares != 0 && price != 0 { mv = shares * price }
                pnl := asFloat(m["pnl"])
                if pnl == 0 && shares != 0 && avg != 0 && price != 0 { pnl = (price - avg) * shares }
                if pnl == 0 && shares != 0 && avg != 0 && mv != 0 { pnl = mv - avg*shares }
                pnlSum += pnl
        ...
        for _, tr := range trades {
                ...
                shares := asFloat(tr["shares"]); price := asFloat(tr["price"]); notional := shares * price
                if side == "buy" { buyN += notional } else if side == "sell" { sellN += notional }
                if len(events) < 12 {
